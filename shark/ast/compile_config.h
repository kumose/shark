...
namespace cppast {
    ...
    enum class CppStandard {
        ...
        cpp_latest = CppStandard::cpp_14, //< The latest supported C++ standard.
        c_latest = CppStandard::c_17, //< The latest supported C standard.
    };

    /// \returns A human readable string representing the option,
    /// it is e.g. `c++14` for `cpp_14`.
    inline const char *to_string(CppStandard standard) noexcept {
        switch (standard) {
            case CppStandard::cpp_98:
                return "c++98";
            case CppStandard::cpp_03:
                return "c++03";
            case CppStandard::cpp_11:
                return "c++11";
            case CppStandard::cpp_14:
                return "c++14";
            case CppStandard::cpp_1z:
                return "c++1z";
            case CppStandard::cpp_17:
                return "c++17";
            case CppStandard::cpp_2a:
                return "c++2a";
            case CppStandard::cpp_20:
                return "c++20";
            case CppStandard::cpp_2b:
                return "c++2b";

            case CppStandard::c_89:
                return "c89";
            case CppStandard::c_99:
                return "c99";
            case CppStandard::c_11:
                return "c11";
            case CppStandard::c_17:
                return "c17";
            case CppStandard::c_2x:
                return "c2x";
        }
    ...
    inline bool is_c_standard(CppStandard standard) noexcept {
        switch (standard) {
            case CppStandard::cpp_98:
            case CppStandard::cpp_03:
            case CppStandard::cpp_11:
    ...
    /// A [ts::flag_set]() of [cppast::CompileFlag]().
    using compile_flags = type_safe::flag_set<CompileFlag>;

    /// Base class for the configuration of a [cppast::parser]().
    class CompileConfig {
    public:
        /// \effects Sets the given C++ standard and compilation flags.
        void set_flags(CppStandard standard, compile_flags flags = {}) {
    ...
    protected:
        CompileConfig(std::vector<std::string> def_flags) : flags_(std::move(def_flags)) {
        }

        CompileConfig(const CompileConfig &) = default;

        CompileConfig &operator=(const CompileConfig &) = default;

        ~CompileConfig() noexcept = default;
    ...
    private:
        /// \effects Sets the given C++ standard and compilation flags.
        virtual void do_set_flags(CppStandard standard, compile_flags flags) = 0;
