...
namespace cppast {
    ...
    struct VisitorInfo {
        ...
        CppAccessSpecifierKind access; //< The current access specifier.
    ...
    enum VisitorResult : bool {
        ...
        /// \notes This only happens when the event is [cppast::VisitorInfo::container_entity_enter]().
        continue_visit_no_children = false,

        /// Visit should be aborted.
        /// \notes This only happens when the event is not
        /// [cppast::VisitorInfo::container_entity_enter]().
        abort_visit = false,
    };

    /// \exclude
    namespace detail {
        using visitor_callback_t = bool (*)(void *mem, const CppEntity &, VisitorInfo info);
        ...
        visitor_callback_t get_visitor_callback(
            visitor_info_bool,
            decltype(std::declval<Func>()(std::declval<const CppEntity &>(), VisitorInfo{})) = true) {
            return [](void *mem, const CppEntity &e, VisitorInfo info) {
        ...
        visitor_callback_t get_visitor_callback(
            visitor_info_void,
            decltype(std::declval<Func>()(std::declval<const CppEntity &>(), VisitorInfo{}), 0) = 0) {
            return [](void *mem, const CppEntity &e, VisitorInfo info) {
                auto &func = *static_cast<Func *>(mem);
                func(e, info);
                return true;
        ...
        bool visit(const CppEntity &e, visitor_callback_t cb, void *functor,
                   CppAccessSpecifierKind cur_access, bool last_child);
    } // namespace detail

    /// Visits a [cppast::CppEntity]() and children.
    ///
    /// \effects It will invoke the callback - the visitor - for the current entity and children,
    /// as controlled by the [cppast::VisitorResult]().
    ...
    /// If it returns nothing, a return value [cppast::VisitorResult::continue_visit]() is assumed.
    template<typename Func>
    void visit(const CppEntity &e, Func f) {
    ...
    namespace detail {
        using visitor_filter_t = visit_filter (*)(const CppEntity &);

        template<typename Filter>
        auto invoke_visit_filter(int, Filter f, const CppEntity &e, CppAccessSpecifierKind access)
            -> decltype(static_cast<visit_filter>(f(e, access))) {
            return static_cast<visit_filter>(f(e, access));
        }

        template<typename Filter>
        auto invoke_visit_filter(short, Filter f, const CppEntity &e, CppAccessSpecifierKind)
            -> decltype(static_cast<visit_filter>(f(e))) {
            return static_cast<visit_filter>(f(e));
        }
    } // namespace detail

    /// Visits a [cppast::CppEntity]() and children that pass a given filter.
    ///
    ...
    /// The filter must be a function taking a [cppast::CppEntity]() as first parameter and optionally
    /// a [cppast::CppAccessSpecifierKind]() as second. It must return a `bool` or a
    /// [cppast::visit_filter]().
    template<typename Func, typename Filter>
    void visit(const CppEntity &e, Filter filter, Func f) {
        visit(e, [&](const CppEntity &e, const VisitorInfo &info) -> bool {
            ...
            switch (result) {
                ...
                case visit_filter::exclude_and_children:
                    ...
                    if (info.event == VisitorInfo::container_entity_enter)
    ...
    /// \exclude
    namespace detail {
        template<CppEntityKind... K>
        bool has_one_of_kind(const CppEntity &e) {
    ...
    template<CppEntityKind... Kinds>
    detail::visitor_filter_t blacklist() {
        return [](const CppEntity &e) {
    ...
    detail::visitor_filter_t blacklist_and_children() {
        return [](const CppEntity &e) {
            return detail::has_one_of_kind<Kinds...>(e)
                       ? visit_filter::exclude_and_children
