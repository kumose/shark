...
void CppTokenString::builder::unmunch() {
...
namespace {
    ...
    type_safe::optional<CppToken> identifier_token(const char *&ptr) {
        ...
        if (find_keyword != std::end(keywords))
            return CppToken(CppTokenKind::keyword, identifier.value());
        else if (identifier == "and")
            return CppToken(CppTokenKind::punctuation, "&&");
            ...
            return CppToken(CppTokenKind::identifier, identifier.value());
    }
    ...
    type_safe::optional<CppToken> numeric_literal_token(const char *&ptr) {
        if (starts_with(ptr, "0b") || starts_with(ptr, "0B")) // binary integer literal
        {
            ...
            return CppToken(CppTokenKind::int_literal, result);
        } else if (starts_with(ptr, "0x") || starts_with(ptr, "0X")) // hexadecimal literal
        {
            ...
            return CppToken(is_float ? CppTokenKind::float_literal : CppTokenKind::int_literal,
                            result);
        } else if (is_digit(*ptr)) // octal and decimal literals
        {
            ...
            append_floating_point_suffix(result, ptr);
            return CppToken(CppTokenKind::float_literal, result);
        } else
            return type_safe::nullopt;
    }
    ...
    type_safe::optional<CppToken> character_literal(const char *&ptr) {
        auto save = ptr;
        ...
        if (*ptr != '\'') {
            ...
            return CppToken(CppTokenKind::char_literal, result);
        }
    }

    type_safe::optional<CppToken> string_literal(const char *&ptr) {
        auto save = ptr;
        auto prefix = parse_encoding_prefix(ptr);
        if (starts_with(ptr, "R\"")) {
            ...
            append_udl_suffix(result, ptr);
            return CppToken(CppTokenKind::string_literal, result);
    ...
    type_safe::optional<CppToken> digraph_token(const char *&ptr) {
        if (bump_if(ptr, "<%"))
            return CppToken(CppTokenKind::punctuation, "{");
        else if (bump_if(ptr, "%>"))
            return CppToken(CppTokenKind::punctuation, "}");
    ...
    type_safe::optional<CppToken> punctuation_token(const char *&ptr) {
        ...
        for (auto punct: punctuations)
            if (bump_if(ptr, punct))
                return CppToken(CppTokenKind::punctuation, punct);

        return type_safe::nullopt;
    }
} // namespace

CppTokenString CppTokenString::tokenize(std::string str) {
    CppTokenString::builder builder;
...
std::string CppTokenString::as_string() const {
    std::string result;
...
bool cppast::operator==(const CppTokenString &lhs, const CppTokenString &rhs) {
