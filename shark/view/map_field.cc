...
namespace shark {
    MapFieldViewGenerator::MapFieldViewGenerator(const google::protobuf::FieldDescriptor *descriptor)
        ...
        _variables["key_type"] = get_ctype(k, descriptor_->containing_type());
        _variables["value_type"] = get_ctype(v, descriptor_->containing_type());
    }

    // implements FieldGenerator ---------------------------------------
    void MapFieldViewGenerator::generate_members(google::protobuf::io::Printer* printer) const {
        printer->Print(_variables, "$deprecated$shark::MapView<$key_type$,$value_type$> _$name$$default_init$;\n");
    }

    void MapFieldViewGenerator::generate_members_declares(google::protobuf::io::Printer *printer) const {
        printer->Print(_variables, "$deprecated$inline shark::MapView<$key_type$,$value_type$> $name$() const;\n");
    }
    ...
    void MapFieldViewGenerator::generate_members_inline_implementations(google::protobuf::io::Printer *printer) const {
        ...
        printer->Print(_variables, "inline shark::MapView<$key_type$,$value_type$> $domain_view$::$name$() const {\n");
        printer->Indent();
