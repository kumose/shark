#include <shark/toml/fwd/datetime_fwd.h>
...
namespace shark {
    ...
    TOML11_INLINE_VERSION_NAMESPACE {
        ...
        local_date::local_date(const std::time_t t)
        ...
        local_date::operator std::time_t() const {
        ...
        local_time::operator std::chrono::nanoseconds() const {
        ...
        time_offset::operator std::chrono::minutes() const {
        ...
        local_datetime::local_datetime(const std::chrono::system_clock::time_point &tp) {
            const auto t = std::chrono::system_clock::to_time_t(tp);
        ...
        local_datetime::local_datetime(const std::time_t t)
        ...
        offset_datetime::offset_datetime(const local_datetime &ld)
            : date{ld.date}, time{ld.time}, offset{get_local_offset(nullptr)}
        // use the current local timezone offset
        {
        }

        offset_datetime::offset_datetime(const std::chrono::system_clock::time_point &tp)
        ...
        offset_datetime::offset_datetime(const std::time_t &t)
            : offset{0, 0} // use gmtime
        {
        ...
        offset_datetime::offset_datetime(const std::tm &t)
        ...
        offset_datetime::operator std::time_t() const {
            return std::chrono::system_clock::to_time_t(
                std::chrono::system_clock::time_point(*this));
        }

        time_offset offset_datetime::get_local_offset(const std::time_t *tp) {
