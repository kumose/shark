#include <shark/toml/fwd/scanner_fwd.h>
...
namespace shark {
    ...
    TOML11_INLINE_VERSION_NAMESPACE {
        namespace detail {
             ...
             region scanner_storage::scan(location &loc) const {
                assert(this->is_ok());
                return this->scanner_->scan(loc);
            }

             std::string scanner_storage::expected_chars(location &loc) const {
                assert(this->is_ok());
                return this->scanner_->expected_chars(loc);
            }

             scanner_base &scanner_storage::get() const noexcept {
                assert(this->is_ok());
                return *scanner_;
            }

             std::string scanner_storage::name() const {
                assert(this->is_ok());
            ...
            // ----------------------------------------------------------------------------
             ...
             region character::scan(location &loc) const {
             ...
             std::string character::expected_chars(location &) const {
                return show_char(value_);
            }

             scanner_base *character::clone() const {
            ...
            // ----------------------------------------------------------------------------
             ...
             region character_either::scan(location &loc) const {
                if (loc.eof()) { return region{}; }
             ...
             std::string character_either::expected_chars(location &) const {
                assert(this->value_);
                assert(this->size_ != 0);
             ...
             scanner_base *character_either::clone() const {
                return new character_either(*this);
            }

             std::string character_either::name() const {
            ...
            // character_in_range
             ...
             region character_in_range::scan(location &loc) const {
                if (loc.eof()) { return region{}; }
             ...
             std::string character_in_range::expected_chars(location &) const {
             ...
             scanner_base *character_in_range::clone() const {
                return new character_in_range(*this);
            }

             std::string character_in_range::name() const {
            ...
            // literal
             ...
             region literal::scan(location &loc) const {
                const auto first = loc;
             ...
             std::string literal::expected_chars(location &) const {
                return std::string(value_);
            }

             scanner_base *literal::clone() const {
            ...
            // sequence
             ...
             region sequence::scan(location &loc) const {
                const auto first = loc;
             ...
             std::string sequence::expected_chars(location &loc) const {
                const auto first = loc;
                for (const auto &other: others_) {
                    const auto reg = other.scan(loc);
                    if (!reg.is_ok()) {
             ...
             scanner_base *sequence::clone() const {
            ...
            // either
             ...
             region either::scan(location &loc) const {
                for (const auto &other: others_) {
                    const auto reg = other.scan(loc);
                    if (reg.is_ok()) {
                        return reg;
             ...
             std::string either::expected_chars(location &loc) const {
             ...
             scanner_base *either::clone() const {
                return new either(*this);
            }

             std::string either::name() const {
                std::string n("either{");
                for (const auto &other: others_) {
                    n += other.name();
                    n += ", ";
            ...
            // repeat_exact
             ...
             region repeat_exact::scan(location &loc) const {
                const auto first = loc;
             ...
             std::string repeat_exact::expected_chars(location &loc) const {
                for (std::size_t i = 0; i < length_; ++i) {
                    const auto reg = other_.scan(loc);
                    if (!reg.is_ok()) {
             ...
             scanner_base *repeat_exact::clone() const {
                return new repeat_exact(*this);
            }

             std::string repeat_exact::name() const {
            ...
            // repeat_at_least
             ...
             region repeat_at_least::scan(location &loc) const {
                const auto first = loc;
                for (std::size_t i = 0; i < length_; ++i) {
                    const auto reg = other_.scan(loc);
                    if (!reg.is_ok()) {
             ...
             std::string repeat_at_least::expected_chars(location &loc) const {
                for (std::size_t i = 0; i < length_; ++i) {
                    const auto reg = other_.scan(loc);
                    if (!reg.is_ok()) {
                        return other_.expected_chars(loc);
             ...
             scanner_base *repeat_at_least::clone() const {
                return new repeat_at_least(*this);
            }

             std::string repeat_at_least::name() const {
            ...
            // maybe
             ...
             region maybe::scan(location &loc) const {
                const auto first = loc;
                const auto reg = other_.scan(loc);
                if (!reg.is_ok()) {
                    loc = first;
             ...
             std::string maybe::expected_chars(location &) const {
                return "";
            }

             scanner_base *maybe::clone() const {
                return new maybe(*this);
            }

             std::string maybe::name() const {
