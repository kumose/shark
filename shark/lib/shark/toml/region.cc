#include <shark/toml/fwd/region_fwd.h>
    ...
    TOML11_INLINE_VERSION_NAMESPACE {
        namespace detail {
            ...
            // Those source must be the same. Instread, `region` does not make sense.
             region::region(const location &first, const location &last)
            ...
            // shorthand of [loc, loc+1)
             region::region(const location &loc)
             ...
             region::char_type region::at(std::size_t i) const {
             ...
             region::const_iterator region::cbegin() const noexcept {
                return std::next(this->source_->cbegin(),
                                 static_cast<difference_type>(this->first_));
            }

             region::const_iterator region::cend() const noexcept {
                return std::next(this->source_->cbegin(),
                                 static_cast<difference_type>(this->last_));
            }

             std::string region::as_string() const {
                if (this->is_ok()) {
            ...
            region::take_line(const_iterator begin, const_iterator end) const {
             ...
             std::vector<std::pair<std::string, std::size_t> > region::as_lines() const {
                assert(this->is_ok());
                if (this->length_ == 0) {
