#include <shark/toml/fwd/location_fwd.h>
...
namespace xconfig {
    ...
    TOML11_INLINE_VERSION_NAMESPACE {
        namespace detail {
             void location::advance(std::size_t n) noexcept {
                assert(this->is_ok());
             ...
             void location::retrace(/*restricted to n=1*/) noexcept {
                assert(this->is_ok());
                if (this->location_ == 0) {
                    this->location_ = 0;
                    this->line_number_ = 1;
             ...
             bool location::eof() const noexcept {
                assert(this->is_ok());
                return this->location_ >= this->source_->size();
            }

             location::char_type location::current() const {
                assert(this->is_ok());
             ...
             location::char_type location::peek() {
                assert(this->is_ok());
             ...
             std::string location::get_line() const {
                assert(this->is_ok());
             ...
             std::size_t location::calc_column_number() const noexcept {
                assert(this->is_ok());
                const auto iter = std::next(this->source_->cbegin(), static_cast<difference_type>(this->location_));
                const auto riter = cxx::make_reverse_iterator(iter);
                const auto prev = std::find(riter, this->source_->crend(), char_type('\n'));
             ...
             void location::advance_impl(const std::size_t n) {
                assert(this->is_ok());
             ...
             void location::retrace_impl(/*n == 1*/) {
                assert(this->is_ok());
