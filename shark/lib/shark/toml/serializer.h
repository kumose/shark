        ...
        namespace detail {
            ...
            private:
                    ...
                    if (fmt.fmt == integer_format::dec) {
                        ...
                        if (i < 0) {
                            throw serialization_error(format_error("binary, octal, hexadecimal "
                                                                   "integer does not allow negative value", loc,
                                                                   "here"), loc);
                        }
                        switch (fmt.fmt) {
                                       ...
                                       const source_location &) // {{{
                {
                    ...
                    oss << string_conv<std::string>(
                        this->format_local_time(odt.time, fmt.has_seconds, fmt.subsecond_precision));
                    oss << odt.offset;
                    return string_conv<string_type>(oss.str());
                } // }}}
                ...
                string_type format_dotted_table(const table_type &t, const table_format_info &fmt, // {{{
                    ...
                    for (const auto &kv: t) {
                        ...
                        if (val.is_table() &&
                            ...
                            retval += this->format_dotted_table(val.as_table(), val.as_table_fmt(), val.location(),
                                                                keys);
