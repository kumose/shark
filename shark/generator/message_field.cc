...
namespace shark {
    ...
    MessageFieldGenerator(const google::protobuf::FieldDescriptor *descriptor, std::string message)
        : FieldGenerator(descriptor, message) {
        variables_["name"] = descriptor_->name();
        variables_["type"] = turbo::str_cat(message, "::", descriptor_->message_type()->name());
    }
    ...
    void MessageFieldGenerator::generate_members_inline_implementations(google::protobuf::io::Printer *printer) const {
        switch (descriptor_->label()) {
            case google::protobuf::FieldDescriptor::LABEL_REQUIRED:
            case google::protobuf::FieldDescriptor::LABEL_OPTIONAL:
                printer->Print(variables_, "inline const $type$& $message$::$name$() const {\n");
                printer->Indent();
                printer->Print(variables_, "return _$name$;\n");
                printer->Outdent();
                printer->Print(variables_, "}\n");
                printer->Print(variables_, "inline void $message$::set_$name$(const $type$& val) {\n");
                printer->Indent();
                printer->Print(variables_, "_$name$ = val;\n");
                printer->Outdent();
                printer->Print(variables_, "}\n");
                printer->Print(variables_, "inline void $message$::set_$name$($type$&& val) {\n");
                printer->Indent();
            ...
            case google::protobuf::FieldDescriptor::LABEL_REPEATED:
                printer->Print(
                    variables_, "inline const std::vector<$type$>& $message$::$name$() const {\n");
                printer->Indent();
                printer->Print(variables_, "return _$name$;\n");
                printer->Outdent();
                printer->Print(variables_, "}\n");
                printer->Print(variables_, "inline std::vector<$type$>& $message$::mutable_$name$() {\n");
                printer->Indent();
                printer->Print(variables_, "return _$name$;\n");
                printer->Outdent();
                printer->Print(variables_, "}\n");
